...
async function onDownloadClick(event) {
    ...
    const downloadHost = "https://r1-ndr.ykt.cbern.com.cn,https://r2-ndr.ykt.cbern.com.cn,https://r3-ndr.ykt.cbern.com.cn".split(",");
    ...
    const detail = (await (await fetch(ramdomItem(detailHost) +
        `/zxx/ndrv2/resources/tch_material/details/${materialId}.json`)).json());
    const pdfDetail = detail.ti_items.find((item) => item.ti_format === "pdf") ?? null;
    if (pdfDetail === null) {
        throw new Error("未找到pdf文件");
    }
    const downloadURL = pdfDetail.ti_storage.replace(/^cs_path:\${ref-path}/, ramdomItem(downloadHost));
    const blob = await (await fetch(downloadURL, {
...
function tchMaterialHook() {
    ...
    //   },
    // });
    const materialUlElement = document.querySelector("#main-content > div.content > div.fish-spin-nested-loading.x-edu-nested-loading > div > div:nth-child(2) > div > div:nth-child(2) > div:nth-child(2) > ul");
    ...
    const callback = (mutationList, observer) => {
        console.log("[SMARTEDU-DOWNLOADER] Mutation Observer Updated", mutationList, observer);
    ...
    async function hook() {
        ...
        const materialSpanDivs = materialUlElement.querySelectorAll("li > div:nth-child(2) >  div:nth-child(1)");
        ...
        for (const materialSpanDiv of materialSpanDivs) {
            ...
            const material = window.materialInfo.find((m) => m.title === materialName);
...
function tchMaterialDetailHook() {
    ...
    const materialSpanDiv = document.querySelector(`#main-content > div.content > div:last-child > div > div > div:nth-child(1)`);
