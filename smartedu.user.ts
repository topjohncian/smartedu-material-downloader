...
interface MaterialDetail {
  ti_items: Array<{
    ti_format: "pdf" | "folder";
    ti_storage: string;
  }>;
}

function ramdomItem<T = any>(arr: T[]): T {
...
function tchMaterialHook() {
  ...
  const materialUlElement: HTMLUListElement | null = document.querySelector(
  ...
  const callback = (
    ...
    const needUpdate = mutationList
      .flatMap(
        (mutation) =>
          [...mutation.addedNodes, ...mutation.removedNodes] as HTMLElement[]
      )
  ...
  async function hook() {
    ...
    const materialSpanDivs: NodeListOf<HTMLDivElement> =
      materialUlElement!!.querySelectorAll(
